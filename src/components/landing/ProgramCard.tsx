...
export default function ProgramCard({ program }: ProgramCardProps) {
  ...
  return (
    <div className="group relative flex flex-col justify-between overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-0 shadow-sm hover:shadow-neon-hover transition-all duration-300 hover:-translate-y-1.5 hover:border-primary/25">
      ...
      <div className="relative aspect-video w-full overflow-hidden bg-slate-50 border-b border-slate-100">
        {displayImageUrl ? (
          ...
          <div className="flex h-full w-full items-center justify-center bg-slate-50/50">
            <GraduationCap className="h-10 w-10 text-slate-300 transition-transform duration-500 group-hover:scale-110" />
          </div>
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-slate-900/10 via-transparent to-transparent" />
        
        {/* Floating Category Tag */}
        <span className="absolute left-4 top-4 inline-flex items-center rounded-lg bg-white/90 backdrop-blur-md px-2.5 py-1 text-[10px] font-bold text-slate-800 border border-slate-100 shadow-sm">
      ...
      <div className="flex-1 flex flex-col justify-between p-6 sm:p-7 bg-white">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          {isHrdCorp && (
            <span className="inline-flex items-center gap-1 rounded-full bg-slate-50 border border-slate-200 px-2 py-0.5 text-[9px] font-bold text-slate-500 uppercase tracking-wide">
        ...
        <div className="mt-4 flex-1">
          ...
          <p className="mt-2.5 text-xs sm:text-sm text-slate-500 line-clamp-3 leading-relaxed font-body">
            {program.description}
          </p>
        </div>
